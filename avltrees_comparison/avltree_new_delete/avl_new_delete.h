...
class CAVLTree {
 private:
    struct leaf {
        ...
        leaf *pRight = nullptr;
        int balanceFactor = 0;
        leaf(T* data) : pData(data), pLeft(nullptr), pRight(nullptr), balanceFactor(0) {}
    };
 ...
 public:
    ...
    void ClearRecursively(leaf* p) {
        if (p != nullptr) {
            ClearRecursively(p->pLeft);
            ClearRecursively(p->pRight);
            delete p;
        }
    }
