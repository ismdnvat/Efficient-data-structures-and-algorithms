...
тип - T;
функция вычисления хеша - uint32_t HashFunc(const T* pElement);
функция сравнения - int32_t Compare(const T *pElement, const T* pElement2).
...
*/
template<class T, uint32_t( *HashFunc)(const T *pElement), int32_t (*Compare)(const T *pElement, const T *pElement2)>
class CHash {
 private:
    /**
    ...
    */
    struct leaf {
        T *pData;
        leaf *pnext;
        leaf(T *_pData, leaf *_pnext) : pData(_pData), pnext(_pnext) {}
        leaf() : leaf(nullptr, nullptr) {};
        virtual ~leaf() { pnext = nullptr; }
    };
 ...
 public:
    ...
    */
    CHash(int32_t hashTableSize, int32_t defaultBlockSize) : m_tableSize(hashTableSize), m_Memory(defaultBlockSize, true) {
    ...
    */
    ...
    bool add(T *pElement) {
        uint32_t index;
        leaf *foundLeaf = findLeaf(pElement, index);
    ...
    */
    ...
    T *find(const T &element) {
        uint32_t index;
        leaf *foundLeaf = findLeaf(&element, index);
        return foundLeaf ? foundLeaf->pData : nullptr;
    }
    ...
    */
    bool remove(const T &element) {
        uint32_t index = HashFunc(&element) % m_tableSize;
        leaf *prev = nullptr;
        for (leaf* curr = m_pTable[index]; curr != nullptr; curr = curr->pnext) {
            if (Compare(curr->pData, &element) == 0) {
                if (prev != nullptr) {
                    prev->pnext = curr->pnext;
                } else {
                    m_pTable[index] = curr->pnext;
                }
 ...
 private:
    ...
    */
    ...
    leaf *findLeaf(const T *pElement, uint32_t& index)
    {
        index = HashFunc(pElement) % m_tableSize;
        leaf *curr = m_pTable[index];
        for (leaf* curr = m_pTable[index]; curr != nullptr; curr = curr->pnext) {
